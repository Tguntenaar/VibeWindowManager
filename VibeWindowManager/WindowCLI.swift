...
enum WindowCLI {
    static func shouldRun(arguments: [String] = CommandLine.arguments) -> Bool {
        ...
        return invocation.executableName == "windows" || !invocation.userArguments.isEmpty
    }
    ...
    static func run(arguments: [String] = CommandLine.arguments) -> Int32 {
        ...
        do {
            ...
            switch command {
            case .help:
                print(helpText(binaryName: invocation.executableName))
                return 0
            case .listApps:
                return listApps()
            ...
            fputs(helpText(binaryName: invocation.executableName), stderr)
            return 2
        }
    }
    ...
    private static func invocationContext(arguments: [String]) -> InvocationContext {
        ...
        let userArguments = Array(arguments.dropFirst()).filter { !$0.hasPrefix("-psn_") }
        return InvocationContext(executableName: executable == "VibeWindowManager" ? "windows" : executable, userArguments: userArguments)
    }
}
